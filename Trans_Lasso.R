...
error.hat <- function(beta, beta.hat){
  error2 <- matrix(NA, nrow = dim(beta.hat)[1], ncol = dim(beta.hat)[2])
  for (i in 1:dim(beta.hat)[1]) {
    error2[i,] <- (beta.hat[i,] - beta)^2
  }
  RMSE <- sqrt(rowMeans(error2))
  PE <- rowSums(error2)
  return(list(RMSE,PE))
}

len.nonzeros <- function(beta, beta.hat){
  a <- c()
  for (i in 1:dim(beta.hat)[1]) {
    a[i] <- length(which(beta.hat[i,]!=0))
  }
  return(mean(a))
}